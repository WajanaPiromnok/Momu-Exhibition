...
public class MainNetworkPlayer : NetworkBehaviour
{
    ...
    [SyncVar(hook = nameof(OnAvatarChanged))] private string avatarOutlookJson;
    ...
    private OutlookChangeHandler outlookChangeHandler;
    ...
    public override void OnStopClient(){
        if(hasAuthority)
            PlaygroundMaster.Instance.ShowBlockCanvas("One account can only have one active login\nPlease try again");
    }
    ...
    public override void OnStartClient()
    {
        ...
        {
            // [Noah]
            // this is when we know a new player is just created
            // we should somehow show the avatar options
        }
    ...
    private void Start() {
        ...
        outlookChangeHandler = GetComponent<OutlookChangeHandler>();
