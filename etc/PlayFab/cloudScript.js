...
handlers.getAllSegments = function (args, context) {
    result = server.GetAllSegments({});
    log.debug(result);
}

handlers.getAllPlayers = function (args, context) {
    request = {SegmentId: "16F8703C2995F907"};
    result = server.GetPlayersInSegment(request);
    log.debug(result);
}

handlers.getAllPlayersEmail = function (args, context) {
    segments = server.GetAllSegments({}).Segments;
    allPlayerSegment = segments.filter(seg => seg.Name === "All Players")[0]
    
    // request = {SegmentId: "16F8703C2995F907"};
    request = {SegmentId: allPlayerSegment.Id};
    segmentData = server.GetPlayersInSegment(request);
    players = segmentData.PlayerProfiles;
    emails = players
        .map(p => p.ContactEmailAddresses)
        .filter(addresses => addresses.length > 0)
        .map(addresses => addresses[0].EmailAddress)
    
    return {asList: emails, asString: emails.join(", ")}
}
...
handlers.setDefaultPlayerData = function(args, context) {
    log.debug('running set default player data')
    var updateResult = server.UpdateUserData({
        PlayFabId: currentPlayerId,
        Data: {
